const keyEquipamentos = 'equipamentos_db'; 
...
function openModal(edit = false, index = 0) {
    ...
    if (edit) {
        sModelo.value = itensEquipamentos[index].modelo;
        sFuncao.value = itensEquipamentos[index].funcao;
        sQuantidade.value = itensEquipamentos[index].quantidade;
        id = index;
    } else {
        sModelo.value = '';
        sFuncao.value = '';
...
function deleteItem(index) {
    ...
    itensEquipamentos.splice(index, 1);
    setItensBD(keyEquipamentos); 
    loadItens(); 
}
...
btnSalvar.onclick = e => {
    ...
    if (id !== undefined) {
        itensEquipamentos[id].modelo = sModelo.value;
        itensEquipamentos[id].funcao = sFuncao.value;
        itensEquipamentos[id].quantidade = sQuantidade.value;
    } else {
...
function loadItens() {
    itensEquipamentos = getItensBD(keyEquipamentos); 
    tbody.innerHTML = '';
    itensEquipamentos.forEach((item, index) => {
...
function setItensBD(key) {
    localStorage.setItem(key, JSON.stringify(itensEquipamentos)); 
}
